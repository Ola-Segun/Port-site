...
import { SunIcon, MoonIcon, GithubIcon, LinkedinIcon, TwitterIcon, MailIcon, MessageCircleIcon, ExternalLinkIcon, MenuIcon, XIcon, ChevronLeftIcon, ChevronRightIcon, SendIcon, Check } from 'lucide-react'
...
const testimonials = [
  ...
  { name: 'Client 2', text: 'Exceeded my expectations. Would definitely work with again.', image: '/placeholder.svg?height=50&width=50' },
  { name: 'Client 3', text: 'Fantastic communication and delivered on time.', image: '/placeholder.svg?height=50&width=50' },
  { name: 'Client 4', text: 'Impressive problem-solving skills and attention to detail.', image: '/placeholder.svg?height=50&width=50' },
  { name: 'Client 5', text: 'Highly recommend for any web development project.', image: '/placeholder.svg?height=50&width=50' },
  { name: 'Client 6', text: 'Excellent work ethic and always meets deadlines.', image: '/placeholder.svg?height=50&width=50' },
  { name: 'Client 7', text: 'A true professional who goes above and beyond.', image: '/placeholder.svg?height=50&width=50' },
]
    ...
    <div className={`min-h-screen ${darkMode ? 'dark bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      ...
      <header ref={headerRef} className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-4 flex flex-col md:flex-row items-center justify-between">
          <div className="md:w-1/2 text-center md:text-left mb-8 md:mb-0">
            ...
            <motion.button
              className={`${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} px-6 py-3 rounded-full font-semibold hover:bg-opacity-90 transition duration-300 flex items-center mx-auto md:mx-0`}
          ...
          >
            <div className="container mx-auto px-4 py-4 flex justify-center space-x-12">
              {['Work', 'Resume', 'Testimonials', 'Contact'].map((item) => (
                <motion.button
                  key={item}
                  className={`${darkMode ? 'bg-gray-800 text-white hover:bg-gray-700' : ' bg-gray-100 text-gray-800 hover:bg-gray-200'} px-4 py-2 rounded-full transition duration-300`}
      ...
      <AnimatePresence>
        {showNav && (
          ...
          >
            <motion.button
              className={`${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'} p-3 rounded-full shadow-lg`}
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
            ...
            <AnimatePresence>
              {navOpen && (
                <motion.div
                  ...
                  className={`${darkMode ? 'bg-white' : 'bg-gray-800'} absolute top-full left-1/2 transform -translate-x-1/2 mt-2 rounded-lg shadow-lg`}
                >
                  {['Work', 'Resume', 'Testimonials', 'Contact'].map((item) => (
                    <motion.button
                      key={item}
                      className={`${darkMode ? 'text-gray-800 hover:bg-gray-300' : ' text-white hover:bg-gray-600'} px-4 py-2 transition duration-300 w-full`}
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onMouseEnter={() => {
                        setCursorVariant('button')
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {currentProjects.map((project, index) => (
              <motion.div
                key={project.title}
                className={`${darkMode ? 'bg-gray-700 border-gray-500' : ' bg-gray-200 border-gray-300'} rounded-lg overflow-hidden shadow-lg cursor-pointer border-b-4`}
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className="px-6 py-3">
                  ...
                  <p className={`${darkMode ? 'text-gray-300' : 'text-gray-500'} mb-2 text-sm`}>{project.description}</p>
                  <div className="flex flex-wrap">
                    {project.languages.map((lang) => (
                      <span key={lang} className={`${darkMode ? 'bg-gray-200 text-gray-800 ' : 'bg-gray-700 text-gray-200'} px-2 py-1 rounded text-sm mr-2 mb-2`}>{lang}</span>
                    ))}
                  </div>
                </div>
              </motion.div>
                ...
                <div className="px-4 py-2 flex justify-between items-center">
                  ...
                  <div className="flex flex-wrap">
                    {selectedProject.languages.map((lang) => (
                      <span key={lang} className="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-2 py-1 rounded-full text-sm mr-2 mb-2 border border-gray-500 h-fit ">
                        {lang}
                      </span>
                    ))}
                  </div>
      ...
      {/* Resume Section */}
      <section id="resume" className={`${darkMode ? 'bg-gray-800' : 'bg-gray-100'} py-20` }>
        <div className="container mx-auto px-4">
        <motion.h2
            className="text-4xl font-bold mb-8 text-center"
            initial={{ opacity: 0, y: 50 }}
            ...
            <div>
              ...
              <div className="flex flex-wrap">
                {tools.map((tool, index) => (
                  <motion.span
                    key={tool}
                    className="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-3 py-1 rounded-full text-sm mr-2 mb-2"
                    initial={{ opacity: 0, scale: 0 }}
          ...
          <div className="flex flex-wrap justify-center">
            {currentTestimonials.map((testimonial, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
              >
                <div className={`${darkMode ? 'bg-gray-700 border-gray-500' : ' bg-gray-200 border-gray-300'} p-6 rounded-lg shadow-lg border-b-4`}>
                  <p className={`${darkMode ? 'text-gray-200' : 'text-gray-700'} mb-4`}>{testimonial.text}</p>
      ...
      {/* Contact Section */}
      <section id="contact" className={`${darkMode ? 'bg-gray-800' : 'bg-gray-100'} py-20` }>
        <div className="container mx-auto px-4">
      ...
      {/* Dark Mode Toggle */}
      <motion.button
        className={`${darkMode ? 'bg-gray-800' : 'bg-gray-300'} fixed bottom-4 right-4 p-2 rounded-full`}
